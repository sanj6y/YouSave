...
windw = tk.Tk()
windw.title("YouSave")
icon=PhotoImage(file=resource_path(resource_path("YouSaveIcon.png")))
windw.iconphoto(False,icon)
windw.configure(background="white")
windw.resizable(False,False)
ac=str(random.randint(1,20))
b=str(random.randint(20,38))
def find_file():
    aaa=filedialog.askdirectory(title="Select folder to download videos to:")
    return aaa
def create_folder():
    ad=find_file()
    global ac
